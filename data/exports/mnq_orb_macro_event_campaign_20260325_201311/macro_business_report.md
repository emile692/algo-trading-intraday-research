...
## Verdict
- Hard filtering improves survivability: **no**. Best hard filter: `skip_all_high_impact` | pass `0.0%` | max-loss breach `0.0%` | days to pass `nan` | expected net/day `-23.33`.
- Hard filtering hurts passing speed too much: **yes**.
- Deleveraging is superior to hard filtering: **yes**. Best deleverage: `deleverage_all_high_impact_0.25x` | pass `0.0%` | max-loss breach `0.0%` | days to pass `nan` | expected net/day `-23.33`.
- Recommended policy for Topstep 50K: **Recommend `baseline` (baseline) on this overlap window.** Best overall row: `baseline` | pass `0.0%` | max-loss breach `0.0%` | days to pass `nan` | expected net/day `-23.33`.
