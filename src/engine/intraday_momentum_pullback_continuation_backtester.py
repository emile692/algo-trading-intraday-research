"""Dedicated backtester for the IMPC intraday strategy."""
...
from src.strategy.intraday_momentum_pullback_continuation import DEFAULT_BAR_MINUTES, IMPCVariantConfig


@dataclass(frozen=True)
class IMPCBacktestResult:
    """Structured IMPC backtest outputs reused by the campaign."""
    ...
    record.update(
        {
            ...
            "signal_pullback_depth": float(open_trade["signal_pullback_depth"]),
            "signal_ema_spread": float(open_trade["signal_ema_spread"]),
...
def run_intraday_momentum_pullback_continuation_backtest(
    ...
    *,
    ...
    """Backtest the IMPC strategy with strict next-open execution."""
    ...
    for session_date, session_df in signal_df.groupby("session_date", sort=True):
        ...
        for row in session_df.itertuples(index=False, name="Bar"):
            ...
            if current_trade is None:
                ...
                stop_ref_long = pd.to_numeric(pd.Series([getattr(row, "entry_stop_reference_long", np.nan)]), errors="coerce").iloc[0]
                stop_ref_short = pd.to_numeric(pd.Series([getattr(row, "entry_stop_reference_short", np.nan)]), errors="coerce").iloc[0]
                target_r = pd.to_numeric(pd.Series([getattr(row, "entry_target_r", np.nan)]), errors="coerce").iloc[0]
                stop_reference = stop_ref_long if signal_direction == 1 else stop_ref_short

                if signal_direction != 0 and pd.notna(stop_reference) and pd.notna(target_r):
                    entry_price = execution_model.apply_slippage(row_open, signal_direction, is_entry=True)
                    stop_price = float(stop_reference)
                    risk_distance = (entry_price - stop_price) * signal_direction
                    target_r_float = float(target_r)

                    if risk_distance > 0.0 and target_r_float > 0.0:
                        target_price = entry_price + signal_direction * target_r_float * risk_distance
                        if signal_direction == 1 and target_price > entry_price and stop_price < entry_price:
                            current_trade = {
                                ...
                                "signal_pullback_depth": float(
                                    pd.to_numeric(pd.Series([getattr(row, "entry_signal_pullback_depth", np.nan)]), errors="coerce").iloc[0]
                                ),
                                "signal_ema_spread": float(
                                    pd.to_numeric(pd.Series([getattr(row, "entry_signal_ema_spread", np.nan)]), errors="coerce").iloc[0]
                                ),
